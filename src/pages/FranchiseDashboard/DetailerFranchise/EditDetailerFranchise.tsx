...
import {
  ...
  getDetailerSingle,
} from "../../../Api/apiDetailersFranchise";
...
import { useGetSingleDetailer } from "../../../Hooks/useDetailerFranchise";

const EditDetailerFranchise: React.FC = () => {
  ...
  const { data: getSingleDetailerData } = useGetSingleDetailer(id);
  ...
  useEffect(() => {
    if (getSingleDetailerData?.user_permissions) {
      const userPermIds = getSingleDetailerData.user_permissions.map(
        (up) => up.permission_id
      );
      ...
      getPermissions?.forEach((perm) => {
        const key =
          perm.name?.toLowerCase().replace(/\s+/g, "_") || `perm_${perm.id}`;
        initialState[key] = userPermIds.includes(perm.id);
      });

      setPermissions(initialState);
    }
  }, [getSingleDetailerData, getPermissions]);
  ...
  //   console.log("9999999999",getPermissions)

  const mutation = useMutation({
    // mutationFn: AddDetailerFranchise,
    mutationFn: ({ id, formData, franchiseId }) =>
      detailerFranchiseEdit(id, formData, franchiseId),
  ...
  const handleUpdateDetailer = (formValues) => {
    ...
    mutation.mutate({
      id,
      formData,
      franchiseId: getSingleDetailerData?.franchise_id,
    });
  };

  useEffect(() => {
    setValue("role_id", getSingleDetailerData?.role_id);
    setValue("first_name", getSingleDetailerData?.first_name);
    setValue("last_name", getSingleDetailerData?.last_name);
    setValue("email", getSingleDetailerData?.email);
    setValue("number", getSingleDetailerData?.number);
    setValue("about", getSingleDetailerData?.about);
  }, [getSingleDetailerData]);

  console.log("gggg", getSingleDetailerData);

  return (
    <MainLayout>
      <div className="px-[40px] ">
        <h2 className="!text-[32px] mb-[47px] font-medium">Edit Detailer</h2>
        <div className="bg-white p-[20px] rounded-[20px]">
          <form onSubmit={handleSubmit(handleUpdateDetailer)}>
...
export default EditDetailerFranchise;
